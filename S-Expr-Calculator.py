...
log = logging.getLogger("S_Expr-Calc: ")
...
def operateOnNumbers(word):
    ...
    if (word[start_index:len(str_add)]  == str_add):
        ...
        for thenum in newlist:
            result = add(result,int(thenum))
        ...
        for thenum in newlist:
            if(int(thenum) == 0):
                result=0
                break
            result = multiply(result,int(thenum))
    return result
...
def calculate(word):
    log.debug(word)
    """
    Get the count of left paranthesis. This is used to evaluate the expressions and come out of the recursion.
    """
    c=word.count('(')
    """
...
The S-expression when not provided as a command-line argument, an exception will be raised
"""
...
if(len(sys.argv) > 1):
	...
	raise Exception("It looks like the S-Expression is not provided")
